...
export const bookApi = createApi({
  ...
  endpoints: (b) => ({
    ...
    getBooks: b.query<Book[], void>({
      query: () => "/books",
      transformResponse: (response: { success: boolean; message: string; books: Book[] }) => {
    ...
    getBook: b.query<Book, string>({
      query: (id) => `/books/${id}`,
      transformResponse: (response: { success: boolean; message: string; book: Book }) => {
        return response.book; // ✅ Return only the book object
      },
    ...
    // ✅ BORROW BOOK
    borrowBook: b.mutation<void, { book: string; quantity: number; dueDate: string }>({
  ...
  query: () => "/borrow",
  transformResponse: (response: { success: boolean; message: string; borrowSummary: BorrowSummary[] }) => {
